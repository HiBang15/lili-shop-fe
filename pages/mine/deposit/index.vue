<template>
  <view class="wap">
    <u-navbar back-text="" title="Pre-deposit list">
    </u-navbar>
    <view class="wrapper-show-money">
      <view class="money-view">
        <h3>Pre-deposit amount </h3>
      ...
      <swiper class="swiper-box" :current="swiperCurrent">
        <swiper-item class="swiper-item" v-for="index in list.length" :key="index">
          <scroll-view class="scroll-v view-wrapper" enableBackToTop="true" scroll-with-animation scroll-y @scrolltolower="loadMore">
            ...
            <u-empty v-if="datas.length==0" mode="history" text="No record yet" />
...
export default {
  data() {
    return {
      ...
      current: 0,
      swiperCurrent: 0,
      userInfo: "", //User details information
      params: {
        pageNumber: 1,
        pageSize: 10,
        order: "desc",
      },
      datas: [], //Data collection traversed
      rechargeList: "", //Recharge details list
      walletLogList: "", //Wallet change list
      list: [
        // {
        // name: "Recharge Details",
        // },
        {
          name: "Pre-deposit change details",
        },
      ],
    };
  },
  ...
  methods: {
    ...
    /**Paging to get pre-deposit recharge record */
    getRecharge() {
      this.status = "loading";
