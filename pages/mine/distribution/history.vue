...
export default {
  data() {
    return {
      cashLogData: [], //Withdrawal record data collection
      achievementData: [], //Collection of distribution performance data,
      status: "loadmore",
      iconType: "flower",
      empty: false,
      params: {
      ...
      routers: "",
      achParams: {
        distributionId: (this.routers && this.routers.id) || "", //distributor id
        distributionName: (this.routers && this.routers.name) || "", //Distributor name
        distributionOrderStatus: "", //distributor order status
        pageNumber: 1,
        pageSize: 10,
      },
    };
  ...
  onLoad(option) {
    let title;
    option.type == 0? (title = "Distribution Performance"): (title = "Withdrawal Record");

    uni.setNavigationBarTitle({
      title: title, //This is the revised navigation bar text
    });
    this.routers = option;
