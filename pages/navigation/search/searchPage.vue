<template>
  <view class="content">
    <u-navbar :background="navObj" :is-back="false">
      <!-- mSearch component If you use the original style, delete the component element -->

      <mSearch ref="mSearch" class="mSearch-input-box" @clickLeft="back" :mode="2" :placeholder="defaultKeyword"
               @search="doSearch(false)" @input="inputChange" @ confirm="doSearch(false)"
      ...
      <div class="keyword-box" v-show="!isShowKeywordList">
        <view class="keyword-block add1">
          <view class="keyword-list-header">
            <view class="u-tips">Popular searches</view>
          </view>
        ...
        <view class="keyword-block" v-if="oldKeywordList.length> 0">
          <view class="keyword-list-header">
            <view class="u-tips">Search History</view>
          ...
          <div class="oldKeyList">
            ...
            <div @click="showMore" v-if=" oldKeywordIndex> loadIndex" class="oldKeyItem">Show more</div>
          </div>
        </view>

        <div class="clear mp-iphonex-bottom" @tap="oldDelete">Clear search history</div>
      </div>
    </view>
    <!-- 搜索 -->
    <view class="goods-content" v-if="isShowSeachGoods">
      <view class="navbar">
        <view class="nav-item" :class="{ current: filterIndex === 0 }" @click="tabClick(0)">Comprehensive sorting</view>
        ...
        <view class="nav-item" @click="sortGoods">Filtering</view>
      </view>
      <!-- One product display per line -->
      <div v-if="isSWitch">
              ...
              <u-col :span="8" @click.native="navigateToDetailPage(item)" class="switchType2">
                ...
                <div class="promotion">
                  <div v-for="(promotionItem,promotionIndex) in getPromotion(item)" :key="promotionIndex">
                    ...
                    <span v-if="promotionItem.indexOf('COUPON') != -1">voucher</span>
                    <span v-if="promotionItem.indexOf('FULL_DISCOUNT') != -1">Full reduction</span>
                    <span v-if="promotionItem.indexOf('SECKILL') != -1">Seckill</span>
                  </div>
                </div>
                <div style="overflow: hidden" class="countConfig">
                  <span style="float: left; font-size: 22rpx">Sold {{ item.buyCount || '0' }}</span>
                  <span style="float: right; font-size: 22rpx">{{ item.commentNum || '0' }} comments</span>
                </div>
              </u-col>
            ...
            <view v-for="(item, index) in goodsList" :key="index" class="goods-item"
              ...
              <view class="goods-detail">
                ...
                <div class="countConfig">
                  <span>Sold {{ item.buyCount || '0' }}</span>
                  <span>{{ item.commentNum || '0' }} comments</span>
                </div>
                <div class="storeSellerName">
                  <div class="textHidden">
                    <u-tag style="margin-right: 10rpx" size="mini" mode="dark" v-if="item.selfOperated"
                           text="self-employed" type="error" />
                    <span>{{ item.storeName || 'Nothing' }}</span>
                  </div>
                  <span>
      ...
      <view class="sort-box ">
        <view class="sort-list">
          <view class="sort-item">
            <view class="sort-title"> Brand</view>
            ...
            <!-- <u-empty v-else text="No brand yet" mode="list"></u-empty> -->
          </view>
          <view class="sort-item">
            <view class="sort-title"> All categories</view>
            ...
            <!-- <u-empty v-else text="No categories yet" mode="list"></u-empty> -->
          </view>
        </view>
        <view class="sort-list">
          <view class="sort-item">
            <view class="sort-title"> Price range</view>
            <view style="display:flex; margin-top:20rpx; align-items: center;">
              <view class="sort-brand-item uinput">
                <view class="sort-radius">
                  <u-input v-model="minPrice" type="number" placeholder="lowest price" input-align="center" />
                </view>
              </view>
              <view>-</view>
              <view class="sort-brand-item uinput">
                <view class="sort-radius">
                  <u-input v-model="maxPrice" type="number" placeholder="maximum price" input-align="center" />
                </view>
              </view>
            </view>
          </view>
        ...
        <view class="sort-btn mp-iphonex-bottom">
          <view class="sort-btn-repick" @click="repick">Reset</view>
          <view class="sort-btn-confim" @click="sortConfim">OK</view>
        </view>
      </view>
    </u-popup>
  </view>
...
export default {
  ...
  methods: {
    ...
    loadDefaultKeyword() {
      ...
      this.defaultKeyword = 'Please enter the search product';
    },
    ...
    oldDelete() {
      uni.showModal({
        content: 'OK to clear search history？',
        success: (res) => {
          if (res.confirm) {
