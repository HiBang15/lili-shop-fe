  ...
  <view class="recommend" :style="styleWay">
    ...
    <view class="prohibition" style="overscroll-behavior: unset;">
      <view class="header" :style="'height:' + header.height +'px;'">
        ...
        {{ title }}
        <view @click="goCart" class="right">
          <text class="alifont icon-gouwuche1" style="font-size: 40rpx;"></text>
        </view>
      </view>
    </view>
    ...
    <view class="index-navs">
      <ms-dropdown-item class="index-nav" :class="{'index-nav-active': nav == 1 }" v-model="sortType"
                        @click.native="handledropdown(1)"
                        :hasSlot="true" :title="sortType ||'Comprehensive Sorting'" ref="dropdownItem1">
        <view class="dropdown-item-content">
          <view class="dropdown-list" :class="{'dropdown-list-active': sortType =='Comprehensive sorting' }"
                @click="setSortType('Comprehensive sorting')">Comprehensive sorting
          </view>
          <view class="dropdown-list" :class="{'dropdown-list-active': sortType =='New product sorting' }"
                @click="setSortType('New product sorting')">New product sorting
          </view>
        </view>
      </ms-dropdown-item>
      ...
      <ms-dropdown-item class="index-nav" :class="{'index-nav-active': nav == 2 }" @click.native="handledropdown(2)"
                        v-model="current_tag" :hasSlot="true" :title="current_tag ||'Classification'"
                        ref="dropdownItem2">

        <view class="dropdown-tags">
          <view class="dropdown-tag" :class="{'dropdown-tag-active': current_tag == item }" @click="setTags(item)"
                v-for="(item, index) in tags">{{ item }}
          </view>
          <view class="dropdown-tag cancel-btn" @click="setTags('category')">Cancel</view>
        </view>
      </ms-dropdown-item>
    </view>
      ...
      <view class="index-item" v-for="(item, index) in goods" :key="index">
        <view class="index-item-img" @click="toProduct(item.goods_id)">
          ...
          <view class="index-item-price">
            ...
            <!-- ¥{{ item.price | unitPrice }}{{ item.point?'+' + item.point +'point':'' }} -->
            <span class="tipsMkt">¥{{ item.mktprice }}</span>
          </view>
          <view class="index-item-title-desc">
            <view>Sold {{ item.buy_count }}</view>
            <view>Praise rate {{ item.grade }}%</view>
          </view>
        </view>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    handledropdown(val) {
      ...
      // If there is no value, then all are closed
      if (!val) {
      ...
      this.sale ='';
      this.price ='';
      // If it is equal to 1, click comprehensive sort
      if (val == 1) {
        this.nav = 1;
        this.$refs.dropdownItem2.closePopup();
    ...
    unique(arr) {
      for (var i = 0; i < arr.length; i++) {
        for (var j = i + 1; j < arr.length; j++) {
          if (arr[i] == arr[j]) {
            //The first is equivalent to the second, and the splice method deletes the second
            arr.splice(j, 1);
            j--;
          }
        }
